...
public extension BarcodePickModule {
    ...
    func finishBarcodePickViewHighlightStyleCustomViewProviderViewForRequest(
        ...
        dispatchMain {
            ...
            if let base64String = response?["view"]
                as? String,
                let viewBytes = Data(base64Encoded: base64String),
                let image = UIImage(data: viewBytes)
            {
    ...
    func registerCustomViewProviderListener(viewId: Int, result: FrameworksResult) {
        result.successAndKeepCallback(result: nil)
    }

    func unregisterCustomViewProviderListener(viewId: Int, result: FrameworksResult) {
        result.success()
    }
}
