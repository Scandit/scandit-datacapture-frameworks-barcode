...
public class BarcodeArViewCreationData {
    ...
    static func fromJson(_ viewJson: String) throws -> BarcodeArViewCreationData {
        ...
        if !hasAllRequiredFields {
            throw NSError(
                domain: "BarcodeArViewCreationData",
                code: -2,
                userInfo: [
                    NSLocalizedDescriptionKey:
                        "Unable to create a BarcodeArView. The given json doesn't contain all the required fields. Required fields = [\(Keys.modeKey), \(Keys.viewKey)]"
                ]
            )
        }
